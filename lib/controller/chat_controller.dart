...
class ChatController extends GetxController implements GetxService {
  ...
  required int? conversationID, required int? index}) async {
    ...
    if(notificationBody == null || notificationBody.adminId != null) {
      response = await chatRepo.sendMessage(message, myImages, 0, UserType.admin, null);
    } else if(notificationBody.restaurantId != null) {
      response = await chatRepo.sendMessage(message, myImages, notificationBody.restaurantId, UserType.vendor, conversationID);
    } else if(notificationBody.deliverymanId != null) {
      response = await chatRepo.sendMessage(message, myImages, notificationBody.deliverymanId, UserType.delivery_man, conversationID);
    }
    if (response!.statusCode == 200) {
      _chatImage = [];
      _chatRawImage = [];
