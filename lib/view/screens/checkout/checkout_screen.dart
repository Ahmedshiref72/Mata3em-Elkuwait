...
class CheckoutScreen extends StatefulWidget {
  ...
  const CheckoutScreen(
      {Key? key, required this.fromCart, required this.cartList})
      : super(key: key);
                  ...
                  return GetBuilder<OrderController>(
                    ...
                    if (restController.restaurant != null &&
                      ...
                      double? maximumCharge =
                          restController.restaurant!.selfDeliverySystem == 1
                              ? restController.restaurant!.maximumShippingCharge
                    ...
                    if (restController.restaurant != null) {
                      restaurantSubscriptionActive =
                          restController.restaurant!.orderSubscriptionActive! &&
                              widget.fromCart;
  ...
  Widget topSection(
    ...
    return Container(
      ...
      child: Column(children: [
        _isCashOnDeliveryActive! && restaurantSubscriptionActive
            ? Container(
                width: context.width,
                color: Theme.of(context).cardColor,
                padding: const EdgeInsets.symmetric(
        ...
        SizedBox(
            height: orderController.orderType != 'take_away'
                ? Dimensions.paddingSizeLarge
                : 0),
        (orderController.orderType != 'take_away')
            ? Center(
                child: Text(
                    '${'delivery_charge'.tr}: ${(orderController.orderType == 'take_away' || (orderController.orderType == 'delivery' ? restController.restaurant!.freeDelivery! : true)) ? 'free'.tr : charge != -1 ? PriceConverter.convertPrice(orderController.orderType == 'delivery' ? charge : deliveryCharge) : 'calculating'.tr}',
...
*/
        ...
        /*(orderController.orderType != 'take_away' && Get.find<SplashController>().configModel!.dmTipsStatus == 1 && !orderController.subscriptionOrder) ?
        Container(
          color: Theme.of(context).cardColor,
        ...
        child: CustomButton(
            ...
            onPressed: () {
              ...
              if (_blocController.text.isEmpty) {
                showCustomSnackBar('enter_bloc_number'.tr);
              }
              if (_areaController.text.isEmpty) {
                showCustomSnackBar('enter_area_number'.tr);
              }
              if (_floorController.text.isEmpty) {
                showCustomSnackBar('enter_floor_number'.tr);
              }
              if (_houseController.text.isEmpty) {
                showCustomSnackBar('enter_house_number'.tr);
              }
              ...
              if (!_isCashOnDeliveryActive! &&
                ...
                orderController.placeOrder(
                    PlaceOrderBody(
                      ...
                      apatrment: _apartmentController.text.trim(),
